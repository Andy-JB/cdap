  ...
  public void sendMessages(ProgramRunId programRunId,
    ...
    try {
      ...
      try (Reader reader = new InputStreamReader(urlConn.getInputStream(), StandardCharsets.UTF_8)) {
        RunRecordDetail responseBody = GSON.fromJson(reader, RunRecordDetail.class);
        LOG.debug("Got RunRecordDetail in response body where the stopping time is {}", responseBody.getStoppingTs());
      }
    } finally {
